...
public class MyHashTable {
	...
	public int searchByEmployeeNumber(int employeeNum) {
        ...
        // IMPLEMENTED: removes employee by employee number, if doesn't exist
        // returns null
	public EmployeeInfo returnByEmployeeNumber(int employeeNum, boolean remove){
            int targetBucket = calcBucket(employeeNum);
            for(int j=0; j<buckets[targetBucket].size(); j++){
                if(employeeNum == buckets[targetBucket].get(j).getEmpNum()){
            ...
            return null; // only reaches this line if not already returned an employee
        }
